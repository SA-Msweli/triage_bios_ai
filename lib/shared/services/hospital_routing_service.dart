...
import '../../features/hospital_routing/data/services/hospital_service.dart';
...
class HospitalRoutingService {
  ...
  Future<List<HospitalCapacity>> _getMockHospitalCapacities({
    ...
    return [
      ...
      HospitalCapacity.fromBasic(
        // Using the .fromBasic factory
        id: 'mock_hospital_3',
  ...
  Future<List<HospitalCapacity>> getNearbyHospitals({
    ...
    _logger.i(
      'Fetching nearby hospitals for ($latitude, $longitude) within ${radiusKm}km',
    );

    try {
      // Convert km to miles for HospitalService
      final radiusMiles = radiusKm / 1.60934;

      // Use the updated HospitalService which now uses Firestore
      ...
      // Convert Hospital entities to HospitalCapacity for backward compatibility
      final hospitalCapacities = hospitals.map((hospital) {
        ...
        return hospital.capacity.copyWith(
          id: hospital.id,
          name: hospital.name,
      ...
      _logger.i(
        'Found ${hospitalCapacities.length} hospitals within ${radiusKm}km from Firestore',
      );
      return hospitalCapacities;
    } catch (e) {
      _logger.e(
        'Failed to get hospitals from HospitalService, falling back to mock: $e',
      );

      // Fallback to mock data
      return _getMockHospitalCapacitiesFallback(
        latitude: latitude,
        longitude: longitude,
        radiusKm: radiusKm,
      );
  ...
  Future<HospitalRoutingResult> findOptimalHospital({
    ...
    try {
      ...
      // Try to use the updated HospitalService with Firestore
      try {
        final maxDistanceMiles = maxDistanceKm / 1.60934;
        final requiredSpecialization = specializations.isNotEmpty
            ? specializations.first
            : null;

        final optimalHospital = await _hospitalService.getOptimalHospital(
        ...
        if (optimalHospital != null) {
          // Convert Hospital to HospitalCapacity for compatibility
          final distance = _calculateDistance(
            patientLatitude,
            patientLongitude,
          ...
          final hospitalCapacity = optimalHospital.capacity.copyWith(
            id: optimalHospital.id,
            name: optimalHospital.name,
            latitude: optimalHospital.latitude,
            longitude: optimalHospital.longitude,
            distanceKm: distance,
          );
          ...
          final scoredHospital = ScoredHospital(
            hospital: hospitalCapacity,
            score: score,
          );

          // Get alternative hospitals
          ...
          final alternatives = nearbyHospitals
              .where((h) => h.id != optimalHospital.id)
              .take(2)
              .toList();

          _logger.i(
            'Optimal hospital selected from Firestore: ${optimalHospital.name}',
          );

          return HospitalRoutingResult(
            recommendedHospital: hospitalCapacity,
            score: score,
            alternativeHospitals: alternatives,
            routingMetrics: _calculateRoutingMetrics(scoredHospital),
          );
        }
      } catch (e) {
        _logger.w(
          'Failed to get optimal hospital from HospitalService, falling back to mock: $e',
        );
      }

      // Fallback to original mock implementation
      return _findOptimalHospitalFallback(
        patientLatitude: patientLatitude,
        patientLongitude: patientLongitude,
        severityScore: severityScore,
        specializations: specializations,
  ...
  Future<HospitalScore> _calculateHospitalScore({
    ...
    double specializationScore =
        0.3; // Base score if specific specializations are needed but hospital data is limited
    if (specializations.isEmpty ||
        specializations.any((s) => s.toLowerCase() == 'general')) {
      specializationScore =
          0.7; // Higher if general care is acceptable or no specifics needed
    } else {
      // Heuristic: Check if hospital name contains any requested specialization keywords
      // This is a basic mock approach.
      if (specializations.any(
        (reqSpec) =>
            hospital.name.toLowerCase().contains(reqSpec.toLowerCase()),
      )) {
  ...
  double _calculateSeverityScore(
    ...
    if (patientSeverity >= 8.0) {
      return hospital.hasEmergencyCapacity && hospital.emergencyBeds > 0
          ? 1.0
          : 0.2;
  ...
  double _calculateOptimizationScore(
    ...
    if (hospital.hasEmergencyCapacity && hospital.emergencyBeds > 0) {
      score -= 10;
    }
  ...
  Future<void> initialize() async {
    try {
      ...
      _logger.i(
        'Hospital routing service initialized successfully (mock mode)',
      );
    } catch (e) {
      _logger.e(
        'Failed to initialize hospital routing service (mock mode): $e',
      );
      rethrow;
    }
  }
  ...
  // Fallback methods for when HospitalService/Firestore is unavailable
  Future<List<HospitalCapacity>> _getMockHospitalCapacitiesFallback({
    required double latitude,
    required double longitude,
    required double radiusKm,
  }) async {
    _logger.w('Using fallback mock hospital capacities');
  ...
  Future<HospitalRoutingResult> _findOptimalHospitalFallback({
    required double patientLatitude,
    required double patientLongitude,
    required double severityScore,
    required List<String> specializations,
    ...
    _logger.w('Using fallback optimal hospital selection');
    ...
    if (hospitalsFromMock.isEmpty) {
      throw RoutingException('No mock hospitals available.');
    }

    final hospitals = hospitalsFromMock
        .map((h) {
    ...
    if (scoredHospitals.isEmpty) {
      throw RoutingException('No suitable hospital found after scoring.');
    }

    final optimalHospital = scoredHospitals.first;
    _logger.i(
      'Optimal hospital selected (fallback): ${optimalHospital.hospital.name}',
    );
